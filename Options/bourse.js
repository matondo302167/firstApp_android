  ...
  const [_scroll_y] = useState(new Value(0));
    ...
    const _pickDocument = async () => {
      ...
      async function uploadImageAsync(uri) {
        ...
        const ref = firebase
          .storage()
          .ref()
          .child('/bourse/'+'/'+firebase.auth().currentUser.email+'/'+desc.name)
    ...
    return(
      <SafeAreaView style={styles.safe_area_view}>
        ...
        />
        <Text style={{color:'#fff',fontSize:20}}>Bourse</Text>
        </Animated.View>
        <Animated.ScrollView
          style={[
            styles.scroll_view,
        ...
        >
          <View style={styles.footer}>
            ...
            Pour faire la demande de bourse, il faut présenter un document 
            indispensable appelé Reditto (revenu familial) de l'année précédente.{'\n'} 
            Pour nous envoyer la demande, nous vous conseillons de l'insérer en format pdf.
            </Text>
