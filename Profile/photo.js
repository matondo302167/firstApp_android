  ...
  const [_scroll_y] = useState(new Value(0));
    ...
    const _pickDocument = async () => {
      ...
      async function uploadImageAsync(uri) {
        ...
        const ref = firebase
          .storage()
          .ref()
          .child('/profil_pictcure/'+'/'+firebase.auth().currentUser.email+'/'+desc.name)
    ...
    return(
      <SafeAreaView style={styles.safe_area_view}>
        ...
        />
        <Text style={{color:'#fff',fontSize:20}}>Ajoutez votre Photo</Text>
        </Animated.View>
        <Animated.ScrollView
          style={[
            styles.scroll_view,
        ...
        >
          <View style={styles.footer}>
            <Text style={[styles.textSign,{color:'#000',marginBottom:10}]}>
            Votre photo de type passeport nous sera nécessaire 
            pour effectuer d'autres procédures. Prière de bien vouloir 
            insérer votre photo.
            </Text>
            <TouchableOpacity onPress = {_pickDocument} >
              <Text style={{alignItems:'center'}}>cliquez sur l'icon pour choisir une photo</Text>
              <FontAwesome5 
                style={[styles.fake_post,{left:50}]}
                name="user-circle" 
                size={200} 
                color="#232654" />
            </TouchableOpacity>
            <Text style={styles.text_footer}>Nom du fichier: {desc.name? desc.name : ''}</Text>
            <View style={styles.button}>
            <TouchableOpacity
              style={[styles.signIn,{marginTop:30,backgroundColor:'#232654'}]}
                  onPress = {uploadFile}
            >
              {loading.loadingLogin? <ActivityIndicator size="large" color="white"/> : <Text style={[styles.textSign, {color:'#fff'}]}>Actualisez votre photo</Text>}
             </TouchableOpacity>
          </View>
          </View>
        </Animated.ScrollView>
